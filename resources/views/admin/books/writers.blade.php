@component('admin.layouts.content',['title'=>'مدیریت نویسندگان'])
    ...
    <div class="card" style="text-align: right;">
     <div class="card-content px-2">
         <form class="form new-book" method="post" action="{{route('admin.writers.store')}}"  enctype="multipart/form-data">
             @csrf
             <div style="direction: rtl"  class="row p-1 py-3">
                 <div class="col-md-4 col-sm-12">
                     <div class="form-group">
                         <label for="bookTitle">*نام نویسنده(غیرتکراری باشد)‌ :</label>
                         <input type="text" id="bookTitle" class="form-control required" name="title" value="{{old('title')}}" placeholder="اجباری" required>
                     </div>
                 </div>
                 <div class="col-md-4 col-sm-12">
                     <div class="form-group">
                         <label for="bookTitle">توضیح نویسنده :</label>
                 ...
                 <div class="col-md-4 m-auto col-sm-12">
                     <div class="form-group">
                         <button href="" class="btn btn-warning" id="" style="min-width: 150px;border-radius: 15px"  type="submit">
                             <i class="feather icon-plus-circle"></i>ثبت نویسنده
                         </button>
                     </div>
                 </div>
             </div>
        ...
        <div class="card-content table-responsive d-block">
            @if($writers->count() > 0)
                <table class="table  table-hover-animation table-striped mb-0 w-100" style="direction: rtl" id="orders">
                    ...
                    <tr>
                        <th scope="col">#</th>
                        <th scope="col" style="min-width: 80px;">نام نویسنده</th>
                        <th scope="col" style="min-width: 80px;">توضیح نویسنده</th>
                    ...
                    @foreach($writers as $writer)
                        <tr class="">
                            ...
                            <td>
                                <button  data-toggle="modal" data-target="#confirmModal{{$writer->id}}" class="btn btn-danger"> <i class="feather icon-trash"></i> </button>
                            </td>
                        </tr>
                        <div class="modal fade " id="confirmModal{{$writer->id}}" tabindex="-1" role="dialog" aria-labelledby="confirmModal{{$writer->id}}" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
                                <div class="modal-content" style=" position: absolute;top: 30px;direction: rtl">
                                    <div class="modal-header d-flex w-100 align-items-center justify-content-between">
                                        <h5 class="modal-title" id="exampleModalCenterTitle">حذف دسته‌بندی {{$writer->title}}</h5>
