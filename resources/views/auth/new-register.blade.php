@component('site.layout.content',['title'=>'ورود|ثبت نام'])
                ...
                <form class="form" method="post"  action="{{ route('auth.register') }}">
                    ...
                    <div style="direction: rtl" class="row">
                        <div class="col-md-12 d-flex flex-column w-100 align-items-center justify-content-center">
                            <div class="d-flex flex-md-row flex-column align-items-center justify-content-around w-100">
    ...
    {{--                                    <button type="button" class="btn btn-info btn-sm" onclick="$('#captchaContainer>img').attr('src','https://press.persiandade.ir/captcha/image?_=1267098935&amp;_='+Math.random());var captcha=document.getElementById('captcha');if(captcha){captcha.focus()}"> <i class="feather icon-refresh-cw"></i> </button>--}}
                                    ...
                                    {{--                                        <button type="button" class="btn btn-info btn-sm" onclick="$('#captchaContainer>img').attr('src','https://press.persiandade.ir/captcha/image?_=1267098935&amp;_='+Math.random());var captcha=document.getElementById('captcha');if(captcha){captcha.focus()}"> <i class="feather icon-refresh-cw"></i> </button>--}}
                                    <a type="button" style="max-width: 50px;" href="javascript:;" class="btn btn-info btn-sm" onclick="document.getElementById('captchaCode').src='/captcha/default?'+Math.random()"> <i class="feather icon-refresh-cw"></i> </a>
                                    <input type="text" id="captcha" name="captcha" autocomplete="off" class="form-control mt-2" placeholder="کد کپچا را وارد کنید">
                                </div>
